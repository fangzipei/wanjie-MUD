...
`WorldType` 联合类型的成员 SHALL 仅包含在 `WORLD_MECHANICS` 注册表中已实现的类型。SHALL NOT 添加未实现的世界类型到 `WorldType` 中。

#### Scenario: WorldType 与注册表一致性
- **WHEN** 检查 `WorldType` 联合类型
- **THEN** 每个成员 SHALL 在 `WORLD_MECHANICS` 和 `WORLD_DATA` 中有对应条目
- **AND** SHALL NOT 存在仅在类型中定义但未在任何注册表中实现的世界
