...
export interface World {
  ...
  // === 危险与机缘 ===
  /** 世界危险效果列表 */
  dangers: import('../data/worldEffectsData').WorldDanger[];
  /** 世界机缘效果列表 */
  opportunities: import('../data/worldEffectsData').WorldOpportunity[];
  
  // === 奖励系数 ===
  /** 奖励系数 */
  rewardCoefficient: import('../data/worldEffectsData').WorldRewardCoefficient;
}
...
export interface ActiveBattleState {
  ...
  towerFloor?: number;
  /** 爬塔敌人信息（用于战斗结算） */
  towerEnemy?: import('./tower/types').TowerEnemy;
}
...
export interface GameState {
  ...
  // 任务系统状态 - 统一管理各任务系统
  taskSystems?: import('./taskSystem/types').AllTaskSystemsState;
  // 扩展系统 - 势力相关
